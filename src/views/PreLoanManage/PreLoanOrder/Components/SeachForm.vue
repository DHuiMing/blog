  ...
  <section class="search-form">
    <el-form :inline="true" :model="formInline" class="demo-form-inline" size="mini">
      <el-form-item :label="$t('tab.OrderNumber')">
        <el-input style="width: 130px" v-model="formInline.orderNumber" :placeholder="$t('tab.OrderNumber')"></el-input>
      </el-form-item>
      ...
      <el-form-item :label="$t('table.status')" v-if="isShow">
        <el-select style="width: 130px" v-model="formInline.state" :placeholder="$t('tab.All')">
          <el-option
            v-for="(item,index) in searchList.allBorrowState"
            :label="item"
            :key="index"
            :value="index"
          >{{item}}</el-option>
