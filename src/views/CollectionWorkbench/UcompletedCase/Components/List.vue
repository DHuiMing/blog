  ...
  <section>
    ...
    >
      ...
      <el-table-column :label="$t('table.Operating')" fixed="right" width="250px" align="center">
        <template slot-scope="scope">
          <el-button type="primary" size="mini" @click="handleAction('Begin',scope.row)">开始催收</el-button>
        </template>
      </el-table-column>
    </el-table>
      ...
      if (actionTag == "Info") {
        ...
        this.$confirm("是否开始催收", "提示", {
          ...
          .then(() => {
            _this
              .$axios({
                url: "/manage/collSelfOrder/updateOrderState.htm",
                method: "post",
                data: { id: row.id, state: 20 }
              })
              .then(res => {
                _this.getList();
                _this.$message({
